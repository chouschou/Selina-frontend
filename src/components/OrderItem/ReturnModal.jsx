...
const ReturnModal = ({ isOpen, order, onClose, onSuccess, paid }) => {
  ...
  const validate = () => {
    ...
    if (!refundInfo.reason?.trim())
      newErrors.reason = "Vui lòng nhập lý do trả đơn ";
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log("Yêu cầu trả đơn  hàng:", refundInfo);
      if (isLoggedIn && order) {
      ...
      onSuccess();
      toast.success("Gửi yêu cầu trả đơn  hàng thành công!");
      resetForm();
      onClose();
    } catch (error) {
      console.error("Lỗi khi gửi trả đơn :", error);
